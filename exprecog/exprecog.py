...
from exprecog.classes import Peltarion_Emotion_Classifier
    ...
    def __init__(self,
        ...
        if cascade_file is None:
            cascade_file = pkg_resources.resource_filename(
                'exprecog', 'data/haarcascade_frontalface_default.xml')
        ...
        if not emotion_model:
            ...
            emotion_model = pkg_resources.resource_filename(
                'exprecog', 'data/emotion_model.hdf5')
            ...
            self.__emotion_classifier = Peltarion_Emotion_Classifier(
                url, token)
            self.__emotion_target_size = (48, 48)  # Default Exprecog image size
        else:
...
def inexprecogence():
    ...
    exprecog = Exprecog()
    inexprecogence = exprecog.detect_emotion(args.image)
    print(inexprecogence)
